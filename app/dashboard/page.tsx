  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData(response.data); // Ensure data is directly in response.data
      } catch (err) {
...
// Form component
const FormComponent = <T extends Record<string, unknown>>({ onSubmit, formFields, submitButtonText }: FormComponentProps<T>) => {
    ...
    <form className='flex flex-col gap-4' onSubmit={handleSubmit}>
      {Object.entries(formFields).map(([name, value]) => (
        <input
          key={name}
          name={name}
          type={name === 'date' ? 'date' : (name === 'images') ? 'file' : 'text'}
